...
- **Start Command:** `python manage.py migrate --noinput && gunicorn do_fu_ioh.wsgi:application --bind 0.0.0.0:$PORT`

`build.sh` ya ejecuta instalación de dependencias, `collectstatic` y migraciones.
